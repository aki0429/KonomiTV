...
async def ChannelsAPI(
    ...
    # 呼び出し元 (ログイン中のユーザー、または Cookie の匿名 ID) が IPTV ページからテレビ視聴 UI に登録した IPTV チャンネルを追加する
    ## 登録内容は呼び出し元ごとに分離されている
    user_key = await ResolveUserKey(request, response)
    for iptv_channel in IPTVUtil.GetTVUIChannels(user_key):
